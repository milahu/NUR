...
def eval_repo(repo: Repo, repo_path: Path) -> str:
    ...
    with tempfile.TemporaryDirectory(temp_suffix) as d:
        ...
        try:
            ...
            msg = f"evaluation timed out of after {eval_timeout} seconds"
...
def update_command_inner(args: Namespace) -> None:
    ...
    for repo in update_repos:
        ...
        try:
            ...
            #
            # --- the user repo starts here ---
            #
            # … while calling anonymous lambda
            #   at /nix/store/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx-nur-packages/default.nix:1:1:
            ...
            if repo.eval_repo_path != None:
                logger.debug(f"patching eval_repo_path: {str(repo.eval_repo_path)!r} -> '.'")
