...
stdenv.mkDerivation rec {
  pname = "torzu";
  inherit (sources.torzu) version src;
  ...
  cmakeFlags = [
    ...
    "-DTORZU_CHECK_SUBMODULES=OFF"

    # enable some optional features
    "-DTORZU_USE_QT_WEB_ENGINE=ON"
    "-DTORZU_USE_QT_MULTIMEDIA=ON"
    "-DUSE_DISCORD_PRESENCE=ON"

    # We dont want to bother upstream with potentially outdated compat reports
    "-DTORZU_ENABLE_COMPATIBILITY_REPORTING=OFF"
  ...
  postInstall = ''
    install -Dm444 $src/dist/72-torzu-input.rules $out/lib/udev/rules.d/72-torzu-input.rules
  '';
}
