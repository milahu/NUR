...
in
{
  ...
  config = lib.mkIf (cfg.enable && config.sane.maxBuildCost >= 3) {
    ...
    # - OLLAMA_NUM_PARALLEL (default: 1 or 4)
    # - OLLAMA_MAX_QUEUE (default: 512)
    # - OLLAMA_FLASH_ATTENTION (set to 1 to reduce memory usage for applicable models)
    # - OLLAMA_KV_CACHE_TYPE (f16, q8_0, q4_0 -- for when using flash attention)
    # - OLLAMA_DEBUG=1
