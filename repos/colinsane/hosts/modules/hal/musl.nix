...
in
{
  ...
  config = lib.mkIf cfg.enable {
    # XXX(2026-01-14): pkgs.glibcLocales is null for non-gnu systems;
    # the i18n module @ <repo:NixOS/nixpkgs:nixos/modules/config/i18n.nix> expects it to be an actual derivation.
    # TODO: upstream a proper fix; meanwhile, hack-disable the i18n functionality which would cause eval errors.
    i18n.glibcLocales = "/";
    ...
    # sane.programs.bash-language-server.enableFor = { system = false; user.colin = false; };  #< 2026-01-24: blocked on vector
    ...
    sane.programs.celeste64.enableFor.user.colin = false;  #< 2026-01-26: blocked on dotnet-sdk
    ...
    sane.programs.dtrx.enableFor.user.colin = false; #< 2026-01-24 - 2026-02-02: blocked on rpm: `rpmio/rpmglob.c:84:15: error: ‘GLOB_BRACE’ undeclared`
    ...
    sane.programs.gnome-frog.enableFor.user.colin = false;  #< 2026-02-03: blocked on posthog -> anthropic -> ... -> arrow-cpp
    sane.programs.itgmania.enableFor.user.colin = false;  #< 2026-02-03: fails build "error: conflicting declaration of C function ‘void __assert_fail(...)"
    sane.programs.krita.enableFor.user.colin = false;  #< 2026-02-03: blocked on opencolorio (which fails tests)
    sane.programs.libreoffice.enableFor.user.colin = false;  #< 2026-02-03: blocked on rhino -> gradle: "libc.so.6 -> not found"
    sane.programs.losslesscut-bin.enableFor.user.colin = false;  #< 2026-02-03: fails because buildFHSEnv (which uses glibc)
    sane.programs.mepo.enableFor.user.colin = false;  #< 2026-01-29: fails installPhase: "Failed loading SDL3 library."
    sane.programs.newelle.enableFor.user.colin = false;  #< 2026-02-03: blocked on anthropic -> ... -> arrow-cpp
    # sane.programs.nix-index.enableFor.user.colin = false;  #< 2026-01-24: `nix-main` fails compile (TODO: use lix)
    sane.programs.nixd.enableFor = { system = false; user.colin = false; };  #< 2026-01-24: `nix-expr` fails compile (TODO: use lix)
    # sane.programs.nixpkgs-hammering.enableFor.user.colin = false;  #< 2026-01-24: `nix-main` fails compile (TODO: use lix)
    # sane.programs.nixpkgs-review.enableFor.user.colin = false;  #< 2026-01-24: `nix-main` fails compile (TODO: use lix)
    sane.programs.openscad-lsp.enableFor = { system = false; user.colin = false; };  #< 2026-01-24 - 2026-02-02: fails rust linking
    ...
    sane.programs.visidata.enableFor.user.colin = false;  #< 2026-02-02: blocked on python3Packages.aiohttp, arrow-cpp
